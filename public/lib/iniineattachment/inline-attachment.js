...
(function(document, window) {
  ...
  var inlineAttachment = function(options, instance) {
    ...
    this.editor = options.simplemde;
    this.filenameTag = '{filename}';
    this.lastValue = null;
    this.simplemde = options.simplemde;
  };
  ...
  inlineAttachment.prototype.onFileUploadResponse = function(xhr) {
    if (this.settings.onFileUploadResponse.call(this, xhr) !== false) {
      ...
      if (result && filename) {
        ...
        // 选中现该编辑器 而不是原生的editor
        var text = this.simplemde.value()+newValue;
        console.log(text);
        // 插入到simplemde的内容中
        this.simplemde.value(text);
        // this.editor.setValue(text);  // 原本是插入textarea中
