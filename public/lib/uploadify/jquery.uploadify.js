...
$.fn.uploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">撤回</span></div>';
	var defaults = {
	...
	this.each(function(){
	  ...
	  var fileObj = {
		  ...
		  filter: function(files) {		//选择文件组的过滤方法
              // 设置prefix
              // this.defaults['formData']['prefix']=$(window.frames["box_iframe"].document).find('meta[name="prefix"]').attr('content');
              defaults.formData['prefix']=$("#box_iframe").contents().find('meta[name="prefix"]').attr('content');
              // alert($("#box_iframe").contents().find('meta[name="prefix"]').attr('content'));
              var arr = [];
